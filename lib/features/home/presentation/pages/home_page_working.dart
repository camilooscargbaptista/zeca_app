...
class HomePageWorking extends StatefulWidget {
  const HomePageWorking({Key? key}) : super(key: key);

  @override
  State<HomePageWorking> createState() => _HomePageWorkingState();
}

class _HomePageWorkingState extends State<HomePageWorking> {
  ...
  Future<void> _handleGenerateCode() async {
    if (!_vehicleConfirmed || _kmController.text.isEmpty || !_isStationValidated) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Por favor, complete todas as etapas'),
          backgroundColor: Colors.red,
        ),
      );
      return;
